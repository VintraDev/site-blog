...
export function Search() {
    ...
    function hancleQueryChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    return (
        <form className="relative group w-full md:w-60" onSubmit={handleSearch}>
            ...
            {query && (
                <CircleX 
                className={`text-gray-300 w-4 h-4 top-1/2 -translate-y-1/2 absolute right-3`}
                onClick={resetSearch}
                />
            )}
        </form>
